...
> **SDwC** | sdwc-api(backend_api), sdwc-web(web_ui)
...
**SDwC** - Survey-driven project documentation generator optimized for AI-assisted development with Claude Code
...
|---------|------|----------------|
| sdwc-api | backend_api | Validate intake YAML, render templates via Jinja2 Template Engine, and deliver generated documentation as ZIP |
| sdwc-web | web_ui | Single-page web interface for uploading intake YAML, previewing output, and downloading generated ZIP |

**Architecture**: monolith / layered

### Project Characteristics
- **Stateless**: No database, no ORM, no migrations. Ignore DB-related patterns in skills.
- **No authentication**: Public utility service. Ignore auth middleware/guard patterns in skills.
- **Single-page UI**: One page with 9 state transitions. Ignore multi-page routing patterns in skills.
...
- **Primary**: opus
- **Secondary**: sonnet
- **Routing**: Opus for architecture decisions and complex logic, Sonnet for routine implementation and tests
...
**I do**: Implement Template Engine pipeline, Write API endpoints and validation logic, Write unit and integration tests
**Human does**: Architecture decisions review, Template rendering accuracy verification
**DO NOT**: Do not modify server resource files (templates, generation_rules, output_contract) without approval

**Decision authority**:
- Autonomous: Implementation details within established architecture | Test case design and coverage | Error message wording | Code refactoring within same module
- [!] Requires approval: New API endpoints or endpoint signature changes | Template Engine rendering pipeline changes | Dependency additions | Database or state management additions

#### sdwc-web - `autonomous`

**I do**: Implement React components, API integration, Basic UI tests
**Human does**: UI/UX review, Visual design decisions
**DO NOT**: Do not add new pages without approval (single-page constraint)

**Decision authority**:
- Autonomous: Component structure and styling | State management implementation | Error display and loading states
- [!] Requires approval: New external library additions | Page layout or navigation changes | API contract changes
...
- All API changes must update Swagger docs
- Every generated file must pass output_contract validation
...
- **Branch strategy**: github_flow - main + feature branches. PR required for merge
- **Commit convention**: conventional
- **PR**: created by both, squash merge
-> Full details: skills/git/

### Testing

- **Approach**: test_after
- unit: pytest (target: 80%)
- integration: pytest
- unit: vitest
- e2e: playwright
